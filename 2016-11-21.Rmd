...
## expected value - continuous case { .build }
...
The continuous definition can be motivated by the notion of "discretizing" the range of $X$ into a partition $\{x_1,x_2,\ldots\}.$ and considering the following approximation, which is again a probability-weighted aveA slightly different view gives the meaning behind book question 4.13:
$$\sum x_i P(x_i < X \le x_{i+1}) = \sum x_i[F(x_{i+i})-F(x_i)]$$
